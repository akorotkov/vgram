...
typedef struct
{
	MemoryContext context, tmpContext;
	HTAB *qgramsHash, *stringQGramsHash, *charactersHash;
	int64 totalCount, totalLength;
} QGramStatState;
...
int qgramTableSize = 0, characterTableSize = 0;
QGramTableElement *qgramTable = NULL, *characterTable = NULL;
...
{
	int mid, cmp, lower = 0, upper = characterTableSize - 1;
...
estimateVGramSelectivilty(const char *vgram)
{
	...
	{
		int lower = 0, upper = qgramTableSize - 1, i;
...
extractVGramsWord(const char *wordStart, const char *wordEnd, void *userData)
{
	...
	{
		const char *r = p;
		int lower = 0, upper = qgramTableSize - 1, len = 0;
...
{
	const char *p = wordStart, *prevR = NULL, *prevP = NULL;
	ExtractVGramsInfo *info = (ExtractVGramsInfo *)userData;
...
extractWords(const char *string, size_t len, WordCallback callback, 
																void *userData)
{
	const char *p, *end = string + len, *firstExtractable = NULL;
	char *lower, *buf;
	size_t lowerLen;
...
qgram_stat_transfn(PG_FUNCTION_ARGS)
{
	...
	if (state == NULL)
    {
		MemoryContext context, aggcontext;
