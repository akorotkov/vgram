...
if [ $CHECK_TYPE = "valgrind" ]; then
	make USE_PGXS=1 VALGRIND=1 TEMP_INSTANCE=1 installcheck || status=$?
	...
	END
		...
		make USE_PGXS=1 TEMP_INSTANCE=1 installcheck || status=$?
else
	make USE_PGXS=1 TEMP_INSTANCE=1 installcheck -j $(nproc) || status=$?
fi
cd ..
